...
from ce_cli.utils import api_call, api_client, download_artifact, notice, \
    format_uuid, resolve_pipeline_runs
...
def application_block():
    block = '''\
        class Application(param.Parameterized):
            pipeline_run_selector = param.ListSelector(default=[], objects=[]) # 1
            hyperparameter_selector = param.ListSelector(default=[], objects=[]) # 2
        
            slicing_metric_selector = param.ObjectSelector(default='', objects=['']) # 3
            ...
            def __init__(self, **params):
                ...
                all_pipelines = api_call(ws_api.get_workspaces_pipelines_api_v1_workspaces_workspace_id_pipelines_get, 
                                         active_workspace)
                ...
                for p in all_pipelines:
                    if p.pipeline_runs is not None and len(p.pipeline_runs)>0:
                        for run in p.pipeline_runs:
                            # This is slowing the comparison down but necessary to update the status of each run
                            r = api_call(
                            ...
                            if r.status == 'Succeeded':
                                ...
                                for s, m in evaluation.slicing_metrics:
                                    result_list.append(dict([('pipeline_name', '{}:{}'.format(p.name, r.id)),
                                ...
                                h_dict = api_call(
                                    ...
                                    p.id,
                                    r.id)
                                h_dict['pipeline_name'] = '{}:{}'.format(p.name, r.id)
            ...
            def _updated_context(self):
                ...
                # get params
                parameter_list = self.hparam_info[self.hparam_info['pipeline_name'].isin(self.pipeline_run_selector)].columns
            ...
            def parameter_graph(self): 
                if self.performance_metric_selector and len(self.hyperparameter_selector) > 0:
                    ...
                    # merge
                    extra_df = pd.merge(self.hparam_info, df, on='pipeline_name', how='left')
                    ...
                    for d in dimensions:
                        try:
                            new_dims.append({'label': d, 'values': pd.to_numeric(extra_df[d])})
                        except:
                            u = sorted(list(extra_df[d].apply(lambda x: str(x)).unique()))
                            mapping = {v: i for i, v in enumerate(u)}
                            new_dims.append({'label': d, 
                                             'tickvals': [mapping[x] for x in u],
                                             'ticktext': u,
                                             'values': extra_df[d].apply(lambda x: str(x)).map(mapping)})
                    
                    final_col = pd.to_numeric(extra_df[self.performance_metric_selector])
